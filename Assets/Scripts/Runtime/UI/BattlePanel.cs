...
#region 生命周期
    ...
    protected void Start()
    {
        ...
        // btnDefend: 防御 → 由 PlayerBattleState 处理（回血回蓝 + 临时护盾）
...
#region 事件监听
    ...
    /// <summary>
    /// 角色受伤效果：立绘抖动；死亡时变黑（仅对存活且可见的角色处理）
    /// </summary>
    private void HandleRoleDamageEffect(BattleEventDefine.RoleDamageEffect msg)
    {
        ...
        StartCoroutine(DelayedSpawnHurtNum(slot.VFXPoint, msg.hurtValue));
    }

    /// <summary>
    /// 敌人受伤效果：立绘抖动；死亡时变黑（仅对存活且可见的敌人处理）
    /// </summary>
    ...
    private IEnumerator DelayedSpawnHurtNum(Transform vfxPoint, int damage)
    {
